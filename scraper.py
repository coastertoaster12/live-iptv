import urllib.request
import re
from bs4 import BeautifulSoup

directory_url = "https://dlhd.pk/24-7-channels.php"

# If streams fail to play, update these proxy variables:
base_start = "https://my-easy-proxy-64s3.onrender.com/proxy/hls/manifest.m3u8?d=https%3A%2F%2Fmy-easy-proxy-64s3.onrender.com%2Fextractor%2Fvideo.m3u8%3Fhost%3Ddlstreams%26d%3Dhttps%253A%252F%252Fdlhd.pk%252Fwatch.php%253Fid%253D"
base_end = "%26redirect_stream%3Dtrue&h_User-Agent=Mozilla/5.0"

# 1. INTERNATIONAL BLOCKLIST (Add keywords here to block other countries/languages)
strict_blocks = [
    "(uk)", " uk ", "uk:", "ca:", "canada", "sky sports", "bt sport", "super sport", 
    "viaplay", "optus", "stan sport", "premier sports", "astrosport", "bein sports", 
    "osn", "supersport", "foxtel", "frenet", "telekom", "dazn", "movistar", "canal+",
    "skynet", "flow sports", "melita", "nova", "cosmote", "tring", "ziggo"
]
espn_intl_blocks = [" af", " nl", " br", " ar", " mx", "paname", "carribean", "brazil", "holland", "sub", "espana"]

# 2. BROAD USA KEYWORD ALLOWLIST (Ensures all US channels pass through)
us_keywords = [
    "usa", "us", "philly", "philadelphia", "bally", "tcm", "tlc", "travel", "vice", "wgn", 
    "pix11", "turner", "cw", "mtv", "vh1", "animal planet", "oxygen", "tv land", "sec", "acc", 
    "big ten", "cinemax", "starz", "showtime", "bravo", "amc", "paramount", "history", "discovery",
    "hgtv", "food", "weather", "newsmax", "pbs", "hbo", "golf", "tennis", "fox", "espn", "cbs", 
    "nbc", "abc", "gsn", "game show", "comedy", "disney", "nick", "cartoon", "fxx", "fx", "tnt", "tbs",
    "cozi", "criterion", "hallmark", "lifetime", "ion", "realtv", "gettv", "metv", "own", "we tv"
]

print("Compiling stripped-down US-only playlist...")

try:
    req = urllib.request.Request(directory_url, headers={'User-Agent': 'Mozilla/5.0'})
    with urllib.request.urlopen(req, timeout=10) as response:
        html = response.read()

    soup = BeautifulSoup(html, 'html.parser')
    m3u_output = '#EXTM3U\n'
    found_count = 0

    for a_tag in soup.find_all('a', href=True):
        href = a_tag['href']
        match = re.search(r'watch\.php\?id=(\d+)', href)
        if match:
            channel_id = match.group(1)
            raw_channel_name = a_tag.get_text(strip=True)
            name_lower = raw_channel_name.lower()

            # Filter international blocks immediately
            if any(block in name_lower for block in strict_blocks):
                continue
            if "espn" in name_lower and any(intl in name_lower for intl in espn_intl_blocks):
                continue

            # Check if it fits the broad US criteria
            is_us = (
                any(k in name_lower for k in us_keywords) or 
                name_lower.endswith(" us") or 
                name_lower.endswith(" usa")
            )
            if not is_us:
                continue

            # Clean display name for your layout guide
            display_name = re.split(r'id\s*:', raw_channel_name, flags=re.IGNORECASE)[0].strip()
            display_name = display_name.rstrip('- ').rstrip('|').strip()

            # Output completely clean layout metadata without logo links
            m3u_output += f'#EXTINF:-1 group-title="USA",{display_name}\n'
            m3u_output += f"{base_start}{channel_id}{base_end}\n"
            found_count += 1

    with open("playlist.m3u", "w", encoding="utf-8") as f:
        f.write(m3u_output)

    print(f"\nSuccess! Generated playlist with {found_count} US channels and zero logo logic.")

except Exception as e:
    print(f"\nAn error occurred: {e}")
